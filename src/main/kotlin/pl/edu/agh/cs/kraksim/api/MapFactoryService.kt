...
class MapFactoryService(
    ...
    fun from(entity: MapEntity, stateType: StateType): Triple<List<NagelRoad>, List<RoadNode>, List<RoadNode>> =
        when (stateType) {
            StateType.NAGEL_SCHRECKENBERG -> nagelMapFactory.from(entity)
        }

    @Service
    class NagelMapFactory {

        fun from(entity: MapEntity): Triple<List<NagelRoad>, List<Intersection>, List<Gateway>> {
            ...
            val (intersections, gateways) = roadNodes.partitionByType<Intersection, Gateway, RoadNode>()
